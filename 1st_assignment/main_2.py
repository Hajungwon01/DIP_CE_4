...
# FW/BW (Forward/Backward) 변수 초기화
direction = 0  # 0은 정방향, 1은 역방향

# FW/BW 트랙바 콜백 함수
def directionCallback(pos):
    global direction
    direction = pos  # pos 값에 따라 direction 값을 변경합니다.

# FW/BW 트랙바 생성 및 콜백 함수 연결
cv.createTrackbar('FW/BW', 'Video Player : Team 4', 0, 1, directionCallback)

current_frame_index = 0  # 현재 프레임 인덱스 초기화
...
while success:  # Loop until there are no more frames.
    ...
    if direction == 0:  # 정방향 재생
        success, frame = videoCapture.read()
        current_frame_index += 1
        count += 1
    else:  # 역방향 재생
        videoCapture.set(cv.CAP_PROP_POS_FRAMES, current_frame_index - 1)
        success, frame = videoCapture.read()
        current_frame_index -= 1
        count -= 1
        if count < 0:
